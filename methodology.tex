...
This results in a reduction of the resonance and anti-resonance and the corresponding phase shift.  RE does not eliminate the load switching ($\Delta dB$) as shown in Fig.~\ref{fig:trBode}.  The motor velocity $\dot{\theta_a}$ and motor current $i_a$ states are required to implement RE.
...
that the performance of a system is guaranteed to be inside $u$ and $x$, see Figure 31, within a given error ö
and å respectively. When using SMC one, or multiple, parameters are chosen to be unknown. These
